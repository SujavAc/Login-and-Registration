...
let actions = [
  {
    icon: <AiFillFacebook />,
    link: "https://www.facebook.com/sujav.acharya.5",
    name: "Facebook",
  },
  {
    icon: <GrInstagram />,
    link: "https://www.instagram.com/sujav__/",
    name: "Instagram",
  },
  {
    icon: <AiFillYoutube />,
    link:
      "https://www.youtube.com/channel/UCQkbasC7yKwW1NS04uGb5VQ?view_as=subscriber",
    name: "Youtube",
  },
  {
    icon: <GrLinkedin />,
    link: "https://www.linkedin.com/in/sujav-a-72630416b/",
    name: "LinkedIn",
  },
  {
    icon: <FaTwitterSquare />,
    link: "https://twitter.com/acharya_sujav",
    name: "Twitter",
  },
];
...
export class Home extends React.Component {
  constructor(props) {
    ...
    if (token === null) {
      //condition to verify user if they visit the home tab after login or without login by checking the token value
      loggedIn = false;
    }
    this.state = {
      loggedIn,
  ...
  handleClickDialogueOpen = () => {
    /// this function trigger the dialogue box and set the state of setOpen to true.
    this.setState({
      setOpen: true,
    });
  };
  handleDialogueClose = (event, reason) => {
    // this will close the dialogue box.
    this.setState({
      setOpen: false,
    });
  };
  ...
  render() {
    ...
    if (this.state.loggedIn === false) {
      //checked the user state is loggedIn or not.
          ...
          <div className="data">
            ...
            {Data.map((res) => (
              <div key={res.username}>
                {" "}
                {/* it iterater the data array by the key value username */}
                <Card>
                  <CardContent>
                    <Typography>
                      <b>{res.username}</b>
